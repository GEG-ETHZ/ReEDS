...
*===========================================
* --- GEOLOGIC CO2 STORAGE SUPPLY CURVES ---
*===========================================
*remove CO2 storage from valcap and valgen sets because it is not a generation technology
...
scalar ccs_emit_scaler "scaling factor for CCS tech emissions" /9/,
       ...
       model_co2_trans_cost "Cost of CO2 Transportation [$/tCO2]" /%trans_cost_input%/,
       co2_price_set_switch "CO2 price switch" /%GSw_co2price%/,
       co2_storage_compensation_rate "$/tCO2 rate at which CO2 storage facilites are compensated for storing CO2" /%co2_store_credit%/;
...
parameter co2_tax_430t480(t) "--$/metric ton CO2-- co2 tax high"
/
$ondelim
$include inputs_case%ds%co2_tax_430to480ppm.csv
$offdelim
/
;

parameter co2_tax_650t720(t) "--$/metric ton CO2-- co2 tax low"
/
$ondelim
$include inputs_case%ds%co2_tax_650to720ppm.csv
$offdelim
/
;

* set the carbon tax based on switch arguments
if(Sw_CarbTax = 1 and co2_price_set_switch = 0,
emit_tax("CO2",r,t) = co2_tax(t) ;
) ;

if(Sw_CarbTax = 1 and co2_price_set_switch = 2,
emit_tax("CO2",r,t) = co2_tax_430t480(t) ;
) ;


if(Sw_CarbTax = 1 and co2_price_set_switch = 1,
emit_tax("CO2",r,t) = co2_tax_650t720(t) ;
) ;
