...
* CO2 storage variables
  CO2_CAPTURED(r,t)                    "--tCO2-- CO2 captured in a model year"
  GAP_EMISSIONS(r,t)                   "--tCO2-- CO2 captured in between model years"
  INV_CS(co2_storage_bins,r,t)         "--tCO2-- amount of CO2 storage capacity invested in each decision year"
  TOTAL_INV_CS(co2_storage_bins,r,t)   "--tCO2-- cumulative amount of CO2 storage capacity invested in through model year"
  CO2_TRANS(r,rr,t)                    "--tCO2-- cumulative CO2 transported out of balancing area"
  CO2_INJECTED(r,t)                    "--tCO2-- CO2 injected in a given model year"
  TOTAL_CO2_INJECTED(co2_storage_bins,r,t) "--tCO2-- Cumulative amount of CO2 injected through the model year"
  MAX_CO2_TRANS(r,rr,t)                "--tCO2-- maximum amount of CO2 transportation pipeline built"
;
...
* CO2 storage variables
 eq_inv_co2_store(t)      "--tCO2-- investment in CO2 storage capacity in a decision year"
 eq_inv_cco2_store(co2_storage_bins,r,t)   "--tCO2-- cumulative investment in CO2 storage capacity through decision year"
 eq_co2_capture(r,t)      "--tCO2-- amount of CO2 captured in a decision year"
 eq_gap_emissions1(r,t)   "--tCO2-- amount of CO2 emissions captured before first decision years"
 eq_gap_emissions2(r,t)   "--tCO2-- amount of CO2 emissions captured in between decision years"
 eq_cco2_transport(r,t)   "--tCO2-- cumulative amount of captured CO2 transported outside region"
 eq_co2_inj(r,t)          "--tCO2-- amount of CO2 injected in a decision year"
 eq_cco2_inj(r,t)         "--tCO2-- cumulative amount of CO2 injected through a decision year"
 eq_co2_store_link(co2_storage_bins,r,t)  "--tCO2-- link CO2 injected to CO2 storage site investment"
 eq_co2_store_limit(co2_storage_bins,r,t) "--tCO2-- limit on CO2 storage site investment from each bin"
 eq_max_co2_trans1(r,rr,t)"--tCO2-- max CO2 trans 1"
 eq_max_co2_trans2(r,rr,t)"--tCO2-- max CO2 trans 2"
;
...
*===========================================
eq_inv_co2_store(t)$[tmodel(t)]..

     sum{(co2_storage_bins,r)$[rfeas(r)],INV_CS(co2_storage_bins,r,t)} / emit_scale

     =e=

     sum{(r,v,i)$[rfeas(r)$ccs(i)$valcap(i,v,r,t)],
         ccs_emit_scaler * ccs_years_operating * sum{h,hours(h)} * emit_rate("CO2",i,v,r,t) * INV(i,v,r,t) } / emit_scale
;

eq_inv_cco2_store(co2_storage_bins,r,t)$[rfeas(r)$tmodel(t)]..

     TOTAL_INV_CS(co2_storage_bins,r,t) / emit_scale

     =e=

     sum{(tt)$[tmodel_new(tt)$(yeart(tt) <= yeart(t))],INV_CS(co2_storage_bins,r,tt)}  / emit_scale
;

eq_co2_capture(r,t)$[rfeas(r)$tmodel(t)]..

    CO2_CAPTURED(r,t) / emit_scale

    =e=
* CO2 captured equals 90% of the CO2 emissions generated by CCS plants
    sum{(v,h,i)$[ccs(i)$valgen(i,v,r,t)],
         ccs_emit_scaler * hours(h) * emit_rate("CO2",i,v,r,t) * GEN(i,v,r,h,t) } / emit_scale
;

eq_gap_emissions1(r,t)$[rfeas(r)$tfirst(t)]..

   GAP_EMISSIONS(r,t) / emit_scale

   =e=

   0;

eq_gap_emissions2(r,t)$[rfeas(r)$tmodel(t)$(not tfirst(t))]..
   ...
   (yeart(t) - sum(tt$tprev(t,tt), yeart(tt))) * (
                                                  sum{(tt)$[tprev(t,tt)],CO2_CAPTURED(r,tt)} / emit_scale
                                                  + (0.5)*(
                                                           CO2_CAPTURED(r,t) / emit_scale
                                                           - sum{(tt)$[tprev(t,tt)],CO2_CAPTURED(r,tt)} / emit_scale
                                                           )
...
eq_cco2_transport(r,t)$[rfeas(r)$tmodel(t)]..

    GAP_EMISSIONS(r,t)  / emit_scale

    =g=

    sum{rr$rfeas(rr),CO2_TRANS(r,rr,t)} / emit_scale;

eq_co2_inj(r,t)$[rfeas(r)$tmodel(t)]..

    CO2_INJECTED(r,t) / emit_scale

    =e=

    GAP_EMISSIONS(r,t) / emit_scale
    + sum{(rr)$rfeas(rr),CO2_TRANS(rr,r,t)} / emit_scale
    - sum{(rr)$rfeas(rr),CO2_TRANS(r,rr,t)} / emit_scale  ;

eq_cco2_inj(r,t)$[rfeas(r)$tmodel(t)]..

    sum{co2_storage_bins,TOTAL_CO2_INJECTED(co2_storage_bins,r,t)}  / emit_scale

   =e=

   sum{(tt)$[tmodel_new(tt)$(yeart(tt) <= yeart(t))],CO2_INJECTED(r,tt)} / emit_scale
;

eq_co2_store_link(co2_storage_bins,r,t)$[rfeas(r)$tmodel(t)]..
* total investment made:
   TOTAL_INV_CS(co2_storage_bins,r,t) / emit_scale

   =g=

* total CO2 injected up through model year:
  TOTAL_CO2_INJECTED(co2_storage_bins,r,t)  / emit_scale
;

eq_co2_store_limit(co2_storage_bins,r,t)$[rfeas(r)$tmodel(t)]..

*mulitply by 1e3 because the bins are in GtCO2
    co2_storage_supply(r,"cap",co2_storage_bins) * 1e3
...
eq_max_co2_trans1(r,rr,t)$[rfeas(r)$rfeas(rr)$tmodel(t)$(not tfirst(t))]..

    MAX_CO2_TRANS(r,rr,t) / emit_scale

    =g=

    sum{(tt)$[tprev(t,tt)],MAX_CO2_TRANS(r,rr,tt)} / emit_scale;

eq_max_co2_trans2(r,rr,t)$[rfeas(r)$rfeas(rr)$tmodel(t)]..

    MAX_CO2_TRANS(r,rr,t) / emit_scale

    =g=

    CO2_TRANS(r,rr,t) / emit_scale;
